...
public class MoreActivity extends FragmentActivity {
    ...
    private void setFontTypes() {
        ...
        URLSpan urlSpan = new URLSpan(getResources().getString(R.string.contact_email)) {
            @Override
        ...
        URLSpan[] spans = ssbContact.getSpans(0, ssbContact.length(), URLSpan.class);
        for (URLSpan span : spans) {
            int start = ssbContact.getSpanStart(span);
            int end = ssbContact.getSpanEnd(span);
            ssbContact.removeSpan(span);
            span = new URLSpanNoUnderline(span.getURL());
            ssbContact.setSpan(span, start, end, 0);
        }
        //ssbContact.setSpan( urlSpan, 14, 42, Spannable.SPAN_INCLUSIVE_INCLUSIVE );
